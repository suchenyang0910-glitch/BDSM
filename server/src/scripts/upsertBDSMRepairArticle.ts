...
const bodyHtml = sanitizeArticleHtml(`
  ...
  <h2>日常任务与关系修复，要保持边界</h2>
  <p>写下自己的感受、一起整理共用空间、重新安排一次约会或把约定写进清单，可以成为双方自愿的修复方式；前提是它们不影响工作、学习、休息、身体健康、隐私与社会关系，也不会被用来让任何一方感到羞耻或被控制。</p>
  <ul>
    <li><strong>可选择的修复：</strong>用几分钟写下发生了什么、各自需要什么、下次怎样提醒；共同完成一件双方原本就认可的日常事务。</li>
    <li><strong>必须保留的退出权：</strong>任何任务都可拒绝、暂停或改期；不能因拒绝而追加压力、冷落或报复。</li>
    <li><strong>不应越过的边界：</strong>不要干预雇佣关系、公共场所行为、对外联络、医疗与基本生活需求；更不能要求公开暴露或做有安全风险的事情。</li>
  </ul>
  <h2>不应被包装成纠偏的做法</h2>
