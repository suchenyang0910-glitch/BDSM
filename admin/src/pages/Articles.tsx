...
const ArticlesPage: React.FC = () => {
  ...
  const columns: ColumnsType<AdminArticleItem> = [
    ...
    { title: "数据", width: 168, render: (_, row) => <Space size={[4, 4]} wrap><Tag>浏览 {row.metrics?.views || 0}</Tag><Tag>赞 {row.metrics?.likeCount || 0}</Tag><Tag>评 {row.metrics?.commentCount || 0}</Tag><Tag color={row.metrics?.reportCount ? "red" : "default"}>报 {row.metrics?.reportCount || 0}</Tag></Space> },
