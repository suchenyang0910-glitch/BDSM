...
后台的「SEO 关键词」「GEO 关键词」「内容标签」「Telegram 标签」都必须支持一次性粘贴；**逗号分隔是唯一对外约定格式**。英文逗号、中文逗号、换行和分号可作为兼容分隔符，但保存和再次展示时统一为英文逗号加单个空格：`, `。

运营可以直接粘贴如下格式：

```text
trueanal, true anal, lana rhoades, mike adriano, adult video, anal scene, hd adult video, brunette, doggystyle, deepthroat, creampie, cowgirl, reverse cowgirl, rimming, fingering, shaved, natural tits, big cock
```

处理算法必须如下：

1. 按 `,`、`，`、换行、`;`、`；` 切分；去除首尾空格、重复空格和不可见字符；空项丢弃。
2. **SEO/GEO 关键词**保留词组中的单词空格，例如 `adult video`；不得把它拆成两个关键词，也不得自动改写为 hashtag。
3. **内容标签 / Telegram 标签**允许同一套粘贴格式，但在发布预览阶段按 1.3 节转换：去 `#`、空格和标点后再加 `#`。例如 `adult video` 在 Telegram 中显示为 `#adultvideo`。
4. 去重键为 Unicode 规范化 + 小写 + 去首尾空格；首个原始展示值优先保留。`trueanal` 与 `true anal` 在 SEO/GEO 中视为两个不同词组；在 Telegram 标签中均规范化为 `#trueanal`，只保留一个。
5. 单个 SEO/GEO 关键词最大 80 字符；单个 Telegram 标签最大 32 字符；超限项不保存，并返回可定位到原项的校验错误。
6. SEO/GEO 每组最多 20 项；内容标签与 Telegram 标签每组最多 10 项。超出上限必须拒绝保存，不能静默截断。
7. 粘贴、删除或手工编辑后，输入组件应立即显示「有效项 / 上限」和被拒绝项原因；点击保存前将规范化数组提交给服务端，服务端必须重复校验，前端校验不能作为唯一防线。

### 1.5 字段边界（避免误用）

| 字段 | 支持逗号粘贴 | 是否保留词组空格 | 是否进入 Telegram caption |
|---|---:|---:|---:|
| SEO 关键词 | 是 | 是 | 否 |
| GEO 关键词 | 是 | 是 | 否 |
| 内容标签 `tags` | 是 | 否（按 Telegram 规则规范化） | 是 |
| 发布时 `telegramTags` | 是 | 否（按 Telegram 规则规范化） | 是 |

**禁止**把 SEO/GEO 关键词自动复制进 Telegram 标签。运营如需公开标签，必须明确填写内容标签或发布时 Telegram 标签。
...
服务端校验：标题 120、描述 300；SEO/GEO 每组最多 20 个关键词，每个 80 字符，规则以 1.4 节为准。存储、审计和日志不得包含 Telegram 私密邀请链接、频道 ID、钱包地址或用户身份资料。
...
- 关键词输入必须是「可编辑 Tag 输入 + 文本粘贴区」双模式；支持直接粘贴 1.4 节的逗号分隔文本，实时显示有效项、上限和错误项。
- 给出提示：“视频未填写时继承这里的默认值”。
...
- 在发布确认步骤新增“Telegram 标签”输入，支持粘贴逗号分隔词组、`#标签1 #标签2` 或普通词；前端按 1.4 节预校验。
...
10. 粘贴 1.4 节的关键词样例：SEO 数组必须保留 `trueanal` 与 `true anal` 两项；Telegram 预览中两者只生成一个 `#trueanal`，其他合法标签保持顺序。
11. 粘贴 21 个 SEO 关键词或任一超过 80 字符的词组：接口必须 400 并定位错误项；不得静默删项或截断。
